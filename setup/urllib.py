...
"""
# Slightly modified by Edward Akerboom (EA)
...
def getConnectionHandle():
	...
	if not _urlopener:
		...
		return _urlopener.getConnectionHandle()
...
class URLopener:
    ...
    def open_http(self, url, data=None):
        ...
        self.h	= httplib.HTTP(host)
        if data is not None:
            self.h.putrequest('POST', selector)
            self.h.putheader('Content-type', 'application/x-www-form-urlencoded')
            self.h.putheader('Content-length', '%d' % len(data))
        else:
            self.h.putrequest('GET', selector)
        if auth: self.h.putheader('Authorization', 'Basic %s' % auth)
        if realhost: self.h.putheader('Host', realhost)
        for args in self.addheaders: apply(self.h.putheader, args)
        self.h.endheaders()
        if data is not None:
            self.h.send(data)
